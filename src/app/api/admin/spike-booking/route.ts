/**
 * GET /api/admin/spike-booking   ⚠️ TEMPORARY SPIKE — remove before production
 *
 ...
 *
 * Safety:
 *   - Admin-gated by src/middleware.ts (matches /api/admin/*). Only a
 *     signed-in Travelgenix ID admin can reach it.
 *   - Takes lookup values as query params — nothing sensitive is committed.
 *   - Read-only. Touches no database, no mock data, no existing flow.
 *   - Lives on the `live-wiring` branch only.
 *
 * Usage (while signed in to /admin):
 *   /api/admin/spike-booking?ref=ABC123&email=lead@example.com&depart=2026-08-14
 *
 ...
 *   - a flat list of every top-level key + every key seen inside items[]
 *     so the mapping work is obvious at a glance
 */
...
/** Collect the set of keys seen across an array of objects. */
...
export async function GET(req: NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        usage: '/api/admin/spike-booking?ref=ABC123&email=lead@example.com&depart=YYYY-MM-DD',
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { stage: 'credentials', error: (e as Error).message },
      { status: 500 },
    );
  }
  ...
  // Group item keys by product type so the mapping is obvious
  ...
  return NextResponse.json(
    {
      stage: 'success',
      usedAppId: creds.appId, // which Travelify account the demo creds map to
      httpStatus: res.status,
      shapeReport: {
      ...
      rawBooking: parsed, // the FULL response — this is what we map from
    },
    { status: 200 },
  );
}
